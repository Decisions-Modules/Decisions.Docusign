...
namespace Decisions.Docusign
{
    ...
    {        
        ...
        public static DataTypes.DocuSignEnvelopeInformation DeserialiseDocusignEnvelopeInformation(string XML)
        {
            var result = (DataTypes.DocuSignEnvelopeInformation) 
                new XmlSerializer(typeof(DataTypes.DocuSignEnvelopeInformation), "http://www.docusign.net/API/3.0").Deserialize(new StringReader(XML));
            return result;
        }
